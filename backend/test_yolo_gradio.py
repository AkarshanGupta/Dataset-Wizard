...
def create_interface():
    ...
    # Check for pre-trained models in current directory
    for model_file in ["yolov8n.pt", "yolov8s.pt", "yolov8m.pt", "yolov8l.pt", "yolov8x.pt"]:
        if Path(model_file).exists():
            model_paths.append(Path(model_file))
    
    # Convert to strings
    available_models = [str(p) for p in model_paths]
    
    if not available_models:
        available_models = ["yolov8n.pt"]  # Default fallback
    ...
    with gr.Blocks(title="YOLO Model Tester", theme=gr.themes.Soft()) as demo:
        gr.Markdown(
            ...
            Test your trained YOLO models or pre-trained models on custom images.
        ...
        gr.Markdown("*Upload your own images or use sample images if available*")
        ...
        demo.load(
            fn=lambda: tester.load_model(available_models[0]) if available_models else "No models found",
            outputs=model_status
        )
